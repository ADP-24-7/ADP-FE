...
export function AiOperationsOverviewDashboard() {
  ...
  const sourceFieldTotal = data?.dataClassControls.reduce((sum, item) => sum + item.transformedFields + item.retainedFields, 0) ?? 0;
  const transformedFieldTotal = data?.dataClassControls.reduce((sum, item) => sum + item.transformedFields, 0) ?? 0;
  const minimizationRate = sourceFieldTotal ? transformedFieldTotal / sourceFieldTotal * 100 : 0;
  ...
  return <div className="da-overview ai-overview">
    ...
    {data ? <>
      ...
      <div className="da-dashboard-grid ai-minimization-grid">
        <article className="da-dashboard-panel ai-minimization-panel"><header><div><h2>데이터 최소화 효과 (Butterfly)</h2><p>외부 AI 전달 전 데이터 클래스별 원문 필드와 최소화 적용 결과를 비교합니다.</p></div></header><div className="ai-butterfly-head"><span>원본 데이터 필드</span><span>최소화율</span><span>원문 유지 필드</span></div><div className="ai-butterfly">{data.dataClassControls.slice(0, 7).map((item) => { const total = item.transformedFields + item.retainedFields; const rate = total ? item.transformedFields / total * 100 : 0; return <div key={item.dataClass}><strong>{dataClassLabels[item.dataClass] ?? item.dataClass}</strong><span className="ai-butterfly-left"><b>{total.toLocaleString('ko-KR')}</b><i style={{ width: `${total / maxDataClassFields * 100}%` }} /></span><em>{rate > 0 ? `-${rate.toFixed(1)}%` : '0.0%'}</em><span className="ai-butterfly-right"><i style={{ width: `${item.retainedFields / maxDataClassFields * 100}%` }} /><b>{item.retainedFields.toLocaleString('ko-KR')}</b></span>{item.responseFindings > 0 ? <small>응답 탐지 {item.responseFindings.toLocaleString('ko-KR')}건</small> : null}</div>; })}{!data.dataClassControls.length ? <p>조회 기간에 데이터 최소화 필드 증적이 없습니다.</p> : null}</div></article>
        <article className="da-dashboard-panel ai-minimization-summary"><header><div><h2>민감정보 최소화율</h2><p>원문 유지 없이 변환·마스킹된 필드 비율</p></div></header><div className="ai-minimization-donut" style={{ '--ai-minimization-rate': `${minimizationRate * 3.6}deg` } as CSSProperties}><div><strong>{minimizationRate.toFixed(1)}%</strong><span>최소화 적용</span></div></div><p>최소화 <strong>{transformedFieldTotal.toLocaleString('ko-KR')}</strong> / 원본 <strong>{sourceFieldTotal.toLocaleString('ko-KR')}</strong> 필드</p><div className="ai-minimization-note"><ShieldCheck size={17} /><span>외부 AI에는 정책상 필요한 범위의 데이터만 전달됩니다.</span></div></article>
