    ...
    <div className="da-overview">
      ...
      {data ? <>
        ...
        <article className="da-dashboard-panel da-recent-panel">
          ...
          <footer className="da-execution-pagination"><span>총 {data.recentExecutions.totalElements.toLocaleString('ko-KR')}건 · {data.recentExecutions.page + 1} / {Math.max(1, data.recentExecutions.totalPages)} 페이지</span><div><button type="button" aria-label="이전 페이지" disabled={overview.isFetching || data.recentExecutions.page === 0} onClick={() => setExecutionPage((page) => Math.max(0, page - 1))}><ChevronLeft size={15} /></button><button type="button" aria-label="다음 페이지" disabled={overview.isFetching || data.recentExecutions.page + 1 >= data.recentExecutions.totalPages} onClick={() => setExecutionPage((page) => page + 1)}><ChevronRight size={15} /></button></div></footer>
        </article>
      </> : null}
    </div>
  );
