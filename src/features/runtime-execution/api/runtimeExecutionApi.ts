...
export async function getRuntimeExecution(executionId: string): Promise<RuntimeExecutionDetail> {
  const response = await httpClient.get<RuntimeExecutionDetail>(`/v1/runtime/executions/${executionId}`);
  return response.data;
}
