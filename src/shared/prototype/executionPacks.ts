...
export const executionPacks: ExecutionPack[] = [
  ...
  },
  {
    ...
    defaultProcessingContexts: ['AI_USE'],
    gatewayRequest: {
      workloadPlaceholder: 'customer_summary',
      purposePlaceholder: 'CUSTOMER_SUPPORT',
      subjectPlaceholder: 'customer:{masked-id}',
      destinationPlaceholder: 'dest_internal_provider_project_provisional 또는 평가 모델 Destination',
      inputLabel: '사용자 질문',
    ...
    versionContext: [
      ...
      ['Policy Snapshot', '—'],
      ['Execution Pack', 'AI'],
      ['Profile Snapshot', 'Evaluation Bundle model bindings'],
      ['Analysis Artifact', 'AI Evaluation Bundle'],
      ['Dataset Snapshot', 'Evaluation Run provenance'],
    ],
    boundaries: sharedBoundaries,
  },
  {
  ...
  },
  {
    ...
    badge: 'P0-4 CONTRACT',
    descriptor: 'DIGITAL ASSET · CANONICAL CONTRACT',
    priority: 'Phase 2',
    implementation: 'P0-4/P0-5 연결',
    objective: '승인 거래 Reference와 Canonical Outbound Request를 결속하고 검증된 Artifact Candidate를 Runtime 준비 상태로 연결합니다.',
    baseline: '기존 정산 승인, 이상거래 모니터링, 외부기관 보고 기준을 실행 정책으로 연결합니다.',
    executionSurfaces: ['Approved Transaction Reference', 'Canonical Asset Descriptor', 'Outbound Request', 'Artifact Candidate'],
    policyHarness: [
      { title: 'Canonical Binding Policy', description: 'Chain·Asset·Amount·Destination·Beneficiary를 승인 거래와 정확히 결속' },
      { title: 'Destination Scope', description: '승인된 Asset Platform과 업무 목적을 실행 단위로 고정' },
      { title: 'External Result Traceability', description: '외부 실행 결과와 Recovery 근거를 원문 없이 연결' },
    ],
    destinationProfile: [
      ['Provider', 'Mock Asset Platform'],
      ['Destination Profile', 'dest_mock_asset_platform_v1'],
      ['Artifact', 'P0-5 Lifecycle CANDIDATE'],
      ['Runtime Snapshot', 'P0-6 execution-time pinning'],
    ],
    fieldTreatments: [
      ['Customer ID', 'VAULT_TOKEN'],
      ['Account ID', 'VAULT_TOKEN'],
      ['Requested Destination', 'KEEP_EXACT_PROTECTED'],
      ['Asset Descriptor', 'KEEP_EXACT_PROTECTED'],
      ['Atomic Amount', 'KEEP_EXACT_PROTECTED'],
    ],
    evidenceChecks: [
    ...
    runtimeFocus: [
      ['Input', 'ApprovedTransactionReference + OutboundRequest'],
      ['Contract', 'adp-digital-asset-runtime-contract/v1'],
      ['Next Gate', 'P0-6 Snapshot → P0-7 Guard → P0-8 Recovery'],
    ],
    defaultProcessingContexts: ['DIGITAL_ASSET'],
    gatewayRequest: {
      workloadPlaceholder: 'tokenized_asset_purchase',
      purposePlaceholder: 'DIGITAL_ASSET_PURCHASE',
      subjectPlaceholder: 'customer:{approved-scope}',
      destinationPlaceholder: 'dest_mock_asset_platform_v1',
      inputLabel: 'Asset Purchase Input',
      inputPlaceholder: '현재 BE P0-4 Canonical Runtime input을 사용합니다.',
    },
    versionContext: [
      ['Application', '—'],
      ['Policy Snapshot', '—'],
      ['Execution Pack', 'DIGITAL_ASSET'],
      ['Profile Snapshot', 'P0-6 Runtime Trace'],
      ['Analysis Artifact', 'P0-5 CANDIDATE 조회 가능'],
      ['Dataset Snapshot', '해당 없음'],
    ],
    boundaries: sharedBoundaries,
  },
];
