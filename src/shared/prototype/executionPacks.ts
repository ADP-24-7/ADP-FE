...
export const executionPacks: ExecutionPack[] = [
  ...
  },
  {
    key: 'digital-asset',
    label: 'Digital Asset',
    role: 'Runtime Contract Validation',
    scope: '승인 거래·외부전달·Recovery',
    badge: 'THIN E2E CONTRACT',
    descriptor: 'DIGITAL ASSET · CONTRACT VALIDATION',
    priority: 'Phase 2',
    implementation: 'Thin E2E 검증',
    objective: '현재 Thin E2E 계약에서 승인된 자산 구매 입력과 Asset Platform 외부전달 결과를 검증합니다.',
    baseline: '기존 정산 승인, 이상거래 모니터링, 외부기관 보고 기준을 실행 정책으로 연결합니다.',
    executionSurfaces: ['Purchase Input', 'Approval Scope', 'Asset Platform Payload', 'External Result'],
    policyHarness: [
      { title: 'Purchase Field Policy', description: 'Customer·Account는 토큰화하고 Wallet·Asset·Amount는 정확성을 보존' },
    ...
    destinationProfile: [
      ...
      ['Result', 'External execution status'],
      ['Recovery', 'SENT_UNKNOWN status query'],
    ],
    fieldTreatments: [
      ['Customer ID', 'VAULT_TOKEN'],
      ['Account ID', 'VAULT_TOKEN'],
      ['Wallet Address', 'KEEP_EXACT_PROTECTED'],
      ['Asset ID', 'KEEP_EXACT_PROTECTED'],
      ['Amount', 'KEEP_EXACT_PROTECTED'],
    ],
    evidenceChecks: [
    ...
    runtimeFocus: [
      ['Input', 'Typed Digital Asset Purchase'],
      ['Outbound', 'Destination-specific Asset Platform payload'],
      ['Inbound', 'External result·Recovery·Mismatch evidence'],
    ],
    ...
    gatewayRequest: {
      ...
      inputPlaceholder: '현재 BE Thin E2E의 typed purchase input을 사용합니다.',
    },
    versionContext: [
      ['Application', '—'],
      ['Policy Snapshot', '—'],
